...
function renderAsSame(...schematics: [string, string]): boolean {
	...
	for (let i = 0; i < size; i++) {
		...
		if (configA instanceof Point2 && configB instanceof Point2 && !configA.equals(configB))
			return false;
		if (configA instanceof Item && configB instanceof Item && configA.code != configB.code)
			return false;
		if (configA instanceof Liquid && configB instanceof Liquid && configA.name != configB.name)
			return false;
	}
	return true;
}
export const get: RequestHandler<Context> = async ({ params, context }) => {
	...
	const differentImages =
		change.Changed && original ? !renderAsSame(change.Changed.text, original.text) : false;
	// const differentImages =
