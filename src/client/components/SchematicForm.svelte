...
<div class="wrapper {parseState}">
	<form {action} class:locked data-variant={variant} on:submit={submit} bind:this={form}>
		<div class="mode">
			<button type="button" class:selected={mode == "text"} on:click={() => changeMode("text")}
				>Text</button
			>
			<button type="button" class:selected={mode == "file"} on:click={() => changeMode("file")}>
				File</button
			>
		</div>
		<div class="inputs">
			<label for="name">Name:</label>
			<input name="name" id="name" placeholder="Name of the schematic" required value={name} />
